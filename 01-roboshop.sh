...
for instance in $@
do
    INSTANCE_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t3.micro --security-group-ids $SD_ID --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=AWStest}]" --query 'Instances[0].InstanceId' --output text)
